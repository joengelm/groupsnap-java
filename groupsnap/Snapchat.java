...
public class Snapchat {
    ...
    public void setProxied(Boolean p) {
        proxied = p;
    }
     ...
     */
    public static Snapchat login(String username, String password, Boolean proxy) {
        ...
        try {
            HttpResponse<JsonNode> resp = requestJson(LOGIN_PATH, params, null, proxy);
    ...
    public byte[] getSnap(Snap snap) {
        try {
            ...
            HttpResponse<InputStream> resp = requestBinary(BLOB_PATH, params, null, proxied);
    ...
    public static byte[] getStory(Story story) {
        try {
            HttpResponse<InputStream> resp = requestStoryBinary(STORY_BLOB_PATH + "?story_id=" + story.getId(), true);
            InputStream is = resp.getBody();
            byte[] encryptedBytes = IOUtils.toByteArray(is);
    ...
    public boolean tellIAmTyping(String recipient){
        try {
            ...
            //Make the request
            HttpResponse<String> resp = requestString(CHAT_TYPING_PATH, params, null, proxied);
    ...
    public boolean deleteFriend(String friend){
        try {
            ...
            //Make the request
            HttpResponse<String> resp = requestString(FRIEND_PATH, params, null, proxied);
    ...
    private boolean send(String mediaId, List<String> recipients, boolean story, int time) {
        try {
            ...
            // Execute request
            HttpResponse<String> resp = requestString(path, params, null, proxied);
    ...
    private boolean sendStory(String mediaId, int time, boolean video, String caption) {
        try {
            ...
            // Execute request
            HttpResponse<String> resp = requestString(STORY_PATH, params, null, proxied);
            if (resp.getCode() == 200 || resp.getCode() == 202) {
                return true;
            } else {
    ...
    private boolean updateLoginObj(){
        ...
        try {
            HttpResponse<JsonNode> resp = requestJson(ALL_UPDATES_PATH, params, null, proxied);
            JSONObject obj = resp.getBody().getObject();
            if(obj.has(UPDATES_RESPONSE_KEY) && obj.getJSONObject(UPDATES_RESPONSE_KEY).getBoolean(LOGGED_KEY)){
    ...
    private String upload(File image, boolean video) {
        try {
            ...
            HttpResponse<String> resp = requestString(UPLOAD_PATH, params, encryptedFile, proxied);
            if (resp.getCode() == 200) {
                return mediaId;
            } else {
    ...
    /**
     * Get the base URL for API requests by using a proxy
     *
     * @param proxy a boolean true if the URL should be hidden by proxy, false o/w
     * @return the base URL as a string
     */
    private static String getBaseURL(Boolean proxy) {
        if (proxy) {
            Random rand = new Random();
            int randomNum = rand.nextInt(1000) + 1;
            return "http://www.gmodules.com/ig/proxy?url=https://" + randomNum + ".feelinsonice-hrd.appspot.com/";
        } else {
            return BASE_URL;
        }
    }
    ...
    private static MultipartBody prepareRequest(String path, Map<String, Object> params, File file, Boolean proxy) {
        // Set up a JSON request
        MultipartBody req = Unirest.post(getBaseURL(proxy) + path)
    ...
    private static HttpResponse<InputStream> requestBinary(String path, Map<String, Object> params, File file, Boolean proxy) throws UnirestException {
        MultipartBody req = prepareRequest(path, params, file, proxy);
    ...
    private static HttpResponse<JsonNode> requestJson(String path, Map<String, Object> params, File file, Boolean proxy) throws UnirestException {
        MultipartBody req = prepareRequest(path, params, file, proxy);
    ...
    private static HttpResponse<InputStream> requestStoryBinary(String path, Boolean proxy) throws UnirestException {
        HttpRequest req = Unirest.get(getBaseURL(proxy) + path)
                .header(JSON_TYPE_KEY, JSON_TYPE)
                .header(USER_AGENT_KEY, USER_AGENT);
    ...
    private static HttpResponse<String> requestString(String path, Map<String, Object> params, File file, Boolean proxy) throws UnirestException {
        MultipartBody req = prepareRequest(path, params, file, proxy);
